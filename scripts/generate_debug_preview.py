...
# Default configuration for Debug Preview
SVG_ROOT = "tests/snapshots/debug.svg"
SAMPLES_ROOT = "tests/samples"
OUTPUT_FILE = "preview_debug.html"
EXTENSION = ".debug.svg"

def get_xml_path(svg_full_path):
    parts = svg_full_path.split(os.sep)
    try:
        # Find where 'debug.svg' is in the path to get the relative part
        svg_idx = parts.index('debug.svg')
        sub_path = parts[svg_idx+1:]
        filename_base = sub_path[-1]

        if filename_base.endswith(EXTENSION):
            filename_base = filename_base[:-len(EXTENSION)]
...
def read_xml_content(xml_path):
    if xml_path.endswith(".mid"):
        size = os.path.getsize(xml_path)
        return f"[MIDI source file: {os.path.basename(xml_path)} ({size} bytes) — no MusicXML to display]"
...
def get_file_tree(root_dir):
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        debug_files = [f for f in files if f.endswith(EXTENSION)]

        current_level["_files"] = []
        for f in sorted(debug_files):
            slug = f[:-len(EXTENSION)]
            path_prefix = rel_path if rel_path != "." else ""
            full_path = os.path.join(SVG_ROOT, path_prefix, f)
...
def generate_preview():
    ...
    <head>
        ...
        <title>staveloom Visual Metadata Debug Preview</title>
        <style>
            body {{
            ...
            .legend {{ margin-top: 10px; font-size: 12px; color: red; }}
        </style>
    </head>
    <body>
        <div id="sidebar">
            <h2 style="font-size: 20px; margin-bottom: 5px;">Metadata Debug</h2>
            <div style="font-size: 12px; color: #888; margin-bottom: 20px;">Red lines indicate beat sync points</div>
            <div id="tree-container"></div>
        </div>

        <div id="content-wrapper">
            <button id="toggle-btn" class="toggle-xml-btn" style="display:none;" onclick="toggleXml()">Show Source</button>

            <div id="content">
                <div id="file-info">
                    <h1 id="current-filename">Select a file to debug</h1>
                    <div id="current-path" class="path-info"></div>
                    <div class="legend">Red Line = Beat Sync Point (Label: beat_number / time_seconds)</div>
                </div>
                <div id="viewer">
    ...
    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write(html)
    print(f"Debug preview page generated: {OUTPUT_FILE}")
